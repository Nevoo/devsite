...
|---|---|---|
| "Mobile lightbox: no next/previous controls — add them" | Buttons and swipe both exist (`Lightbox.tsx:72-91`, `:51-59`). They are hidden on mobile by `.lightbox-nav { display: none }` at ≤640px (`site.css:2157-2159`), with the comment "swipe instead" | Work is **unhide + size + gesture-harden**, not build. Roughly a third of the estimated effort. |
